...
object WhatsWrong2 {
  ...
  def getCEOAndEnterprise(ceo_id: Option[String]): Future[Option[(CEO, Enterprise)]] = {
    ceo_id.flatTraverse { id =>
      (for {
        ceo        <- OptionT(CEODao.byId(id))
        enterprise <- OptionT(EnterpriseDao.byCEOId(id))
      } yield (ceo, enterprise)).value
    }
  }
}
